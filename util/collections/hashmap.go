...
// New returns a new HashMap. Like map[string]interface{}
func New[K comparable, V any](cap int) *HashMap[K, V] {
	m := new(HashMap[K, V])
	m.cap = cap
	sz := 8
	for sz < m.cap {
...
func (m *HashMap[K, V]) detectHasher() {
...
func (m *HashMap[K, V]) set(hash int, key K, value V) (prev V, ok bool) {
...
func (m *HashMap[K, V]) Delete(key K) (prev V, deleted bool) {
	if len(m.buckets) == 0 {
		return prev, false
	}
	hash := m.hash(key)
...
func (m *HashMap[K, V]) remove(i int) {
...
// It's not safe to call or HashSet or Delete while scanning.
func (m *HashMap[K, V]) Scan(iter func(key K, value V) bool) {
	for i := 0; i < len(m.buckets); i++ {
		if m.buckets[i].dib() > 0 {
...
// from the map.
func (m *HashMap[K, V]) GetPos(pos uint64) (key K, value V, ok bool) {
	for i := 0; i < len(m.buckets); i++ {
