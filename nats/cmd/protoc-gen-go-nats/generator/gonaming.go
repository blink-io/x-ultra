...
func (g *generator) goTypeName(protoName string) string {
	...
	if def == nil {
		panic("could not find message for " + protoName)
	}

	var prefix string
	if pkg := g.goPackageName(def.File); pkg != g.genPkgName {
		prefix = pkg + "."
	}
...
func (g *generator) goFileName(f *descriptor.FileDescriptorProto) string {
	name := *f.Name // proto file name
	if ext := path.Ext(name); ext == ".proto" || ext == ".protodevel" {
		name = name[:len(name)-len(ext)] // remove extension
	}
	name += ".nats.go" // add nats extension
	...
	if impPath, _, ok := goPackageOption(f); ok && impPath != "" {
		if g.modulePrefix != "" {
			impPath = strings.TrimPrefix(strings.TrimPrefix(impPath, g.modulePrefix), "/")
		}
