...
const (
	...
	defaultCacheTimeout  = 100 * time.Second // 100 sec
	defaultCacheCapacity = 5000_000          // 5,000 msg/sec * 100 sec = 500,000 elements
...
// Config represents Modify NonceCacheCapacity and NonceCacheTimeout if your service needs to
...
type Config struct {
	...
	// KeyBytes is a key is used by lemma to sign requests. Ignored if
	...
	NonceCache         cache.TTLCache[string]
	NonceCacheTimeout  time.Duration // nonce cache timeout
...
// Signer represents an entity that can be used to sign and authenticate requests.
type Signer struct {
	config     *Config
...
// New return a new Signer. Config can not be nil. If you need control over
...
func New(config *Config) (*Signer, error) {
	...
	// setup nonce cache
	ncache, err := newNonceCache(config.NonceCache, config.NonceCacheTimeout)
	if err != nil {
		return nil, err
	}
...
// SignRequest signs a given HTTP request with signature, nonce, and timestamp.
...
// SignRequestWithKey signs a given HTTP request with signature, nonce, and timestamp. Signs the
...
func checkMAC(secretKey []byte, signVerbAndURI bool, httpVerb string, httpResourceURI string,
	...
	// which needs to be decoded before we can use it
