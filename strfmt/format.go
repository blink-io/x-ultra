		...
		for _, v := range f.data {
			...
			if to == tpe {
				...
				case "ulid":
					...
					return ulid, nil
				default:
					return nil, errInvalidTypeName(v.Name)
				}
			}
		}
		return data, nil
...
func (f *defaultFormats) Parse(name, data string) (interface{}, error) {
	...
	return nil, errInvalidTypeName(name)
}

func errInvalidTypeName(name string) error {
	return fmt.Errorf("%s is an invalid type name", name)
}
