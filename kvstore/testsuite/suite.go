// Package testsuite the kvstore tests suite.
package testsuite
...
func RunTestCommon(t *testing.T, kv kvstore.Store) {
	t.Helper()
...
func RunTestListLock(t *testing.T, kv kvstore.Store) {
	t.Helper()
...
// backends.
func RunTestAtomic(t *testing.T, kv kvstore.Store) {
	t.Helper()
...
// by the K/V backends.
func RunTestWatch(t *testing.T, kv kvstore.Store) {
	t.Helper()
...
// by the K/V backends.
func RunTestLock(t *testing.T, kv kvstore.Store) {
	t.Helper()
...
func checkPairNotNil(t *testing.T, pair *kvstore.KVPair) {
	t.Helper()
...
func testPutGetDeleteExists(t *testing.T, kv kvstore.Store) {
	t.Helper()
...
func testWatch(t *testing.T, kv kvstore.Store) {
	t.Helper()
...
func testWatchTree(t *testing.T, kv kvstore.Store) {
	t.Helper()
...
func testAtomicPut(t *testing.T, kv kvstore.Store) {
	t.Helper()
	...
	assert.Equal(t, err, kvstore.ErrKeyModified)
	assert.False(t, success)
}

func testAtomicPutCreate(t *testing.T, kv kvstore.Store) {
	t.Helper()
...
func testAtomicPutWithSlashSuffixKey(t *testing.T, kv kvstore.Store) {
	t.Helper()
...
func testAtomicDelete(t *testing.T, kv kvstore.Store) {
	t.Helper()
...
func testLockUnlock(t *testing.T, kv kvstore.Store) {
	t.Helper()
	...
	lock, err := kv.NewLock(ctx, key, &kvstore.LockOptions{
		Value:          value,
		TTL:            2 * time.Second,
		DeleteOnUnlock: true,
	})
...
func testLockTTL(t *testing.T, kv kvstore.Store, otherConn kvstore.Store) {
	...
	// We should be able to create a new lock on key.
	lockOC, err := otherConn.NewLock(ctx, key, &kvstore.LockOptions{
		Value:     value,
		TTL:       2 * time.Second,
		RenewLock: renewCh,
	})
...
func testPutTTL(t *testing.T, kv kvstore.Store, otherConn kvstore.Store) {
	...
	err := otherConn.Put(ctx, firstKey, firstValue, &kvstore.WriteOptions{TTL: 2 * time.Second})
	require.NoError(t, err)

	// Put a second key with the Ephemeral flag.
	err = otherConn.Put(ctx, secondKey, secondValue, &kvstore.WriteOptions{TTL: 2 * time.Second})
	require.NoError(t, err)
...
func testList(t *testing.T, kv kvstore.Store) {
	t.Helper()
	...
	// Put the parent key.
	err := kv.Put(ctx, parentKey, nil, &kvstore.WriteOptions{IsDir: true})
	require.NoError(t, err)
	...
	err = kv.Put(ctx, subfolderKey, []byte("second"), &kvstore.WriteOptions{IsDir: true})
	require.NoError(t, err)
...
func testListLockKey(t *testing.T, kv kvstore.Store) {
	t.Helper()
	...
	err := kv.Put(ctx, listKey, []byte("val"), &kvstore.WriteOptions{IsDir: true})
	require.NoError(t, err)

	err = kv.Put(ctx, listKey+"/subfolder", []byte("val"), &kvstore.WriteOptions{IsDir: true})
	require.NoError(t, err)
	...
	for i := 1; i <= 3; i++ {
		...
		lock, errPut := kv.NewLock(ctx, key, &kvstore.LockOptions{Value: []byte("locked"), TTL: 2 * time.Second})
		require.NoError(t, errPut)
		require.NotNil(t, lock)
...
func testDeleteTree(t *testing.T, kv kvstore.Store) {
	t.Helper()
...
func RunCleanup(t *testing.T, kv kvstore.Store) {
	t.Helper()
	...
	for _, key := range []string{
		...
		if err != nil {
			assert.ErrorIsf(t, err, kvstore.ErrKeyNotFound, "failed to delete tree key %s", key)
		}

		err = kv.Delete(ctx, key)
		if err != nil {
			assert.ErrorIsf(t, err, kvstore.ErrKeyNotFound, "failed to delete key %s", key)
		}
	}
}
