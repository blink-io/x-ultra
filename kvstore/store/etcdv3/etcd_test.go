...
func makeEtcdV3Client(t *testing.T) kvstore.Store {
	t.Helper()
...
func TestEtcdV3Register(t *testing.T) {
	...
	kv, err := kvstore.NewStore(ctx, Name, []string{client}, nil)
...
func TestEtcdV3KeepAlive(t *testing.T) {
	...
	err := kv.Put(ctx, "foo", []byte("bar"), &kvstore.WriteOptions{
		TTL: 1 * time.Second,
	})
	require.NoError(t, err)
	...
	assert.Error(t, err, kvstore.ErrKeyNotFound)
	assert.Nil(t, pair)
