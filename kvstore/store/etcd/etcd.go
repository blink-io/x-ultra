...
func (s *Store) Get(ctx context.Context, key string, opts *kvstore.ReadOptions) (pair *kvstore.KVPair, err error) {
...
// Put a value at "key".
func (s *Store) Put(ctx context.Context, key string, value []byte, opts *kvstore.WriteOptions) error {
...
func (s *Store) Exists(ctx context.Context, key string, opts *kvstore.ReadOptions) (bool, error) {
...
func (s *Store) Watch(ctx context.Context, key string, opts *kvstore.ReadOptions) (<-chan *kvstore.KVPair, error) {
	wc := etcd.NewWatcher(s.client)

	// respCh is sending back events to the caller.
	respCh := make(chan *kvstore.KVPair)
...
// Providing a non-nil stopCh can be used to stop watching.
func (s *Store) WatchTree(ctx context.Context, directory string, opts *kvstore.ReadOptions) (<-chan []*kvstore.KVPair, error) {
	wc := etcd.NewWatcher(s.client)
	...
	go func() {
		...
		for resp := range watchCh {
			...
			list := make([]*kvstore.KVPair, len(resp.Events))

			for i, ev := range resp.Events {
				list[i] = &kvstore.KVPair{
...
func (s *Store) AtomicPut(ctx context.Context, key string, value []byte, previous *kvstore.KVPair, opts *kvstore.WriteOptions) (bool, *kvstore.KVPair, error) {
	var cmp etcd.Cmp
	var testIndex bool
	...
	if !txn.Succeeded {
		if testIndex {
			return false, nil, kvstore.ErrKeyModified
		}
...
// throws an error if this is the case.
func (s *Store) AtomicDelete(ctx context.Context, key string, previous *kvstore.KVPair) (bool, error) {
	if previous == nil {
	...
	if len(txn.Responses) == 0 {
		return false, kvstore.ErrKeyNotFound
	}

	if !txn.Succeeded {
		return false, kvstore.ErrKeyModified
	}
...
func (s *Store) List(ctx context.Context, directory string, opts *kvstore.ReadOptions) ([]*kvstore.KVPair, error) {
...
func (s *Store) NewLock(ctx context.Context, key string, opts *kvstore.LockOptions) (lock kvstore.Locker, err error) {
	var value string
	ttl := defaultLockTTL
...
func (s *Store) list(ctx context.Context, directory string, opts *kvstore.ReadOptions) (int64, []*kvstore.KVPair, error) {
	ctx, cancel := context.WithTimeout(ctx, etcdDefaultTimeout)
	defer cancel()
	...
	if resp.Count == 0 {
		return 0, nil, kvstore.ErrKeyNotFound
	}
...
func createConfig(addrs []string, options *Config) etcd.Config {
	cfg := etcd.Config{
		Endpoints: kvstore.CreateEndpoints(addrs, "http"),
	}
	...
	if options.TLS != nil {
		cfg.Endpoints = kvstore.CreateEndpoints(addrs, "https")
		cfg.TLS = options.TLS
	}
