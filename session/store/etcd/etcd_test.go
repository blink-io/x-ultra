...
func TestFind(t *testing.T) {
	...
	b, found, err := e.Find(ctx, "session_token")
	if err != nil {
		t.Fatal(err)
	}
	if found != true {
...
func TestSaveNew(t *testing.T) {
	...
	err = e.Commit(ctx, "session_token", []byte("encoded_data"), time.Now().Add(time.Minute))
	if err != nil {
		t.Fatal(err)
	}
...
func TestFindMissing(t *testing.T) {
	...
	_, found, err := e.Find(ctx, "missing_session_token")
	if err != nil {
...
func TestSaveUpdated(t *testing.T) {
	...
	err = e.Commit(ctx, "session_token", []byte("new_encoded_data"), time.Now().Add(time.Minute))
	if err != nil {
		t.Fatal(err)
	}
...
func TestExpiry(t *testing.T) {
	...
	err = e.Commit(ctx, "session_token", []byte("encoded_data"), time.Now().Add(1*time.Second))
	if err != nil {
		t.Fatal(err)
	}

	_, found, _ := e.Find(ctx, "session_token")
	if found != true {
		t.Fatalf("got %v: expected %v", found, true)
	}
...
func TestDelete(t *testing.T) {
	...
	err = e.Delete(ctx, "session_token")
	if err != nil {
		t.Fatal(err)
	}
