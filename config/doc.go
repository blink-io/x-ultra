/*
Package config loads configuration files and/or environment variables into Go structs with extra juice for validating fields and setting defaults.

Config files may be defined in yaml, json or toml format.

When you call `Load()`, config takes the following steps:
...
Define your struct and load it:
	...
	import (
	  ...
	  "github.com/kkyr/config"
	)


	 type Config struct {
	   Build  time.Time `config:"build" validate:"required"`
	   Server struct {
	     Host    string        `config:"host" default:"127.0.0.1"`
	     Ports   []int         `config:"ports" default:"[80,443]"`
	     Cleanup time.Duration `config:"cleanup" default:"30m"`
	   }
	   Logger struct {
...
Pass options as additional parameters to `Load()` to configure config's behaviour.

# IgnoreFile

Do not look for any configuration file with `IgnoreFile()`.

	config.Load(&cfg, config.IgnoreFile())

If IgnoreFile is given then any other configuration file related options like `File` and `Dirs` are simply ignored.

File & Dirs

By default config searches for a file named `config.yaml` in the directory it is run from. Change the file and directories config
searches in with `File()` and `Dirs()`.

	config.Load(&cfg,
	  config.File("settings.json"),
	  config.Dirs(".", "home/user/myapp", "/opt/myapp"),
	)
...
The struct tag key tag config looks for to find the field's alt name can be changed using `Tag()`.
	...
	var cfg Config
	config.Load(&cfg, config.Tag("yaml"))

By default config uses the tag key `config`.
...
Change the layout config uses to parse times using `TimeLayout()`.

	type Config struct {
	  Date time.Time `config:"date" default:"12-25-2019"`
	}

	var cfg Config
	config.Load(&cfg, config.TimeLayout("01-02-2006"))

	fmt.Printf("%+v", cfg)
	// Output: {Date:2019-12-25 00:00:00 +0000 UTC}

By default config parses time using the `RFC.3339` layout (`2006-01-02T15:04:05Z07:00`).

# Strict Parsing

By default config ignores any fields in the config file that are not present in the struct. This behaviour can be changed using `UseStrict()` to achieve strict parsing.
When strict parsing is enabled, extra fields in the config file will cause an error.

# Required

A validate key with a required value in the field's struct tag makes config check if the field has been set after it's been loaded. Required fields that are not set are returned as an error.

	type Config struct {
	  Host string `config:"host" validate:"required"` // or simply `validate:"required"`
	}
...
A default key in the field tag makes config fill the field with the value specified when the field is not otherwise set.

Fig attempts to parse the value based on the field's type. If parsing fails then an error is returned.

	type Config struct {
	  Port int `config:"port" default:"8000"` // or simply `default:"8000"`
	}
...
A wrapped error `ErrFileNotFound` is returned when config is not able to find a config file to load. This can be useful for instance to fallback to a different configuration loading mechanism.
