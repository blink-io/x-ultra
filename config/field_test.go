...
func Test_newStructField(t *testing.T) {
	cfg := struct {
		A int `cfg:"a" default:"5" validate:"required"`
	}{}
	parent := &field{
	...
	f := newStructField(parent, 0, "config")
	if f.parent != parent {
...
func Test_newSliceField(t *testing.T) {
	...
	f := newSliceField(parent, 0, "config")
	if f.parent != parent {
		t.Errorf("f.parent == %p, expected %p", f.parent, f)
	}
	if f.sliceIdx != 0 {
