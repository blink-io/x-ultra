...
const (
	// DefaultFilename is the default filename of the config file that config looks for.
	DefaultFilename = "config.yaml"
	// DefaultDir is the default directory that config searches in for the config file.
	DefaultDir = "."
	// DefaultTag is the default struct tag key that config uses to find the field's alt
	// name.
	DefaultTag = "cfg"
	// DefaultTimeLayout is the default time layout that config uses to parse times.
...
//
// By default, config looks for a file `config.yaml` in the current directory and
// uses the struct field tag `config` for matching field names and validation.
...
//
//	type Config struct {
//	  Env string `config:"env" validate:"required"` // or just `validate:"required"`
//	}
//
...
//
//	type Config struct {
//	  Level string `config:"level" default:"info"` // or just `default:"info"`
//	}
//
...
func (f *config) Load(cfg interface{}) error {
	if !isStructPtr(cfg) {
...
func (f *config) findCfgFile() (path string, err error) {
...
func (f *config) decodeFile(file string) (map[string]interface{}, error) {
	fd, err := os.Open(file)
	if err != nil {
		return nil, err
	}
...
func (f *config) decodeMap(m map[string]interface{}, result interface{}) error {
...
// where applicable.
func (f *config) processCfg(cfg interface{}) error {
...
func (f *config) processField(field *field) error {
...
func (f *config) setFromEnv(fv reflect.Value, key string) error {
...
func (f *config) formatEnvKey(key string) string {
...
// being set.
func (f *config) setDefaultValue(fv reflect.Value, val string) error {
...
func (f *config) setValue(fv reflect.Value, val string) error {
	switch fv.Kind() {
	case reflect.Ptr:
		if fv.IsNil() {
...
func (f *config) setSlice(sv reflect.Value, val string) error {
	ss := stringSlice(val)
